...
export const updateRequestQuery = db.query<
  unknown,
  {
    id: string;
    title: string;
...
export const updateRequestStatusQuery = db.query<
  unknown,
  { id: string; status: RequestStatusEnum }
>(`UPDATE requests SET status = $status WHERE id = $id`);

export const handleRequestAssignmentQuery = db.query<
  unknown,
  { id: string; volunteer_id: string | null; status: RequestStatusEnum }
>(
  `UPDATE requests SET volunteer_id = $volunteer_id, status = $status WHERE id = $id`
);
...
export const findRequestByIdQuery = db.query<
  Request,
  { id: string }
...
export const findRequestsByVolunteerIdQuery = db.query<
  Request,
  { volunteer_id: string }
>(`SELECT *,  json_object(
...
export const findRequestsByStatusQuery = db.query<
  Request,
...
export const findRequestsByBeneficiaryIdQuery = db.query<
  Request,
...
export const findDoneRequestsOfBeneficiaryQuery = db.query<
  Request,
  { beneficiary_id: string }
>(`SELECT *, json_object(
    'address', requests.location_address,
...
export const findDoneRequestsOfVolunteerQuery = db.query<
  Request,
  { volunteer_id: string }
>(`SELECT *, json_object(
    'address', requests.location_address,
...
export const updateUserQuery = db.query<
  unknown,
  {
    username: string;
    email: string;
...
export const findActiveVolunteersQuery = db.query<
  User & { skills: string; availability: UserAvailabilityEnum },
  []
>(
  `SELECT * FROM users u LEFT JOIN volunteers v ON v.user_id = u.id WHERE status = 'active' AND type = 'volunteer' `
);

export const updateVolunteerQuery = db.query<
  unknown,
  { skills: string; availability: string; id: string }
>(
  `UPDATE volunteers SET skills = $skills, availability = $availability WHERE user_id = $id`
);

export const updateBeneficiaryQuery = db.query<
  unknown,
  { needs: string; location: string; id: string }
>(
  `UPDATE beneficiaries SET needs = $needs, location = $location WHERE user_id = $id`
);

export const updateTokensStatusQuery = db.query<
  unknown,
  { userId: string; status: TokenStatusEnum; type: TokenTypeEnum }
>(
  `UPDATE tokens SET status = $status WHERE user_id = $userId AND type = $type`
);
...
export const insertUserRegionQuery = db.query<
  unknown,
